...
### ✅ DESIMAL — **SELESAI (2026-06-29 sesi 9)** *(konten BONUS, belum diukur)*
Region **"Telaga Titik"** 💧 (`dadd`/`dsub` setelah Bilangan Bulat). **1 angka di belakang koma**, hitung di **TENTHS** (integer) → **anti floating-point**: `genDecQ(op,maxT)` simpan `aT`/`bT` integer + `ans:ansT/10`; sub jaga hasil ≥ 0. Jawaban: `parseAnswer(raw,op)` (desimal → `parseFloat(koma→titik)`) + `answerCorrect(v,ans,op)` (`Math.round(v*10)===Math.round(ans*10)`) dipakai di `submitEnc`+`submitGuide` (op-aware, ops lain tetap `parseInt`/`===`). Keypad "," (np-dot) masukin KOMA (ID). qText/guideHint/buildGuideVisual koma. AKM: uang ribuan/meter/liter/kg. Musuh 💧 Si Tetes, 🌊 Si Ombak. **"1" & "1,0" dua-duanya diterima.**
**Verifikasi:** `verify_dec.js` **48/48** — kunci: **round-trip FP end-to-end** (ketik jawaban koma → parse → benar, ×800), "0,1+0,2=0,3" aman, sub tak minus, adaptif 6 tingkat, AKM + harness lama no-regresi + render Edge (combat "0,5 + 0,3 = ?" + input "0,8" via keypad + tombol "," muncul). Backup `matheroes_dec_v1_done.html`.
**BILANGAN kini sangat lengkap:** 5 op + negatif + desimal. **Berikutnya:** Geometri lanjutan (satuan/segitiga/lingkaran/volume) · Aljabar · Data & Ketidakpastian · perkalian/pembagian desimal · pecahan↔desimal · Reasoning PISA.
